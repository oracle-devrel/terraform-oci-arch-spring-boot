...
resource "oci_core_instance" "bastion_instance" {
  ...
  dynamic "shape_config" {
    for_each = local.is_flexible_bastion_instance_shape ? [1] : []
    content {
      memory_in_gbs = var.BastionInstanceFlexShapeMemory
      ocpus         = var.BastionInstanceFlexShapeOCPUS
    }
  }
  ...
  source_details {
    source_type             = "image"
    source_id               = lookup(data.oci_core_images.BastionInstanceImageOCID.images[0], "id")
...
resource "oci_bastion_session" "ssh_via_bastion_service" {
  depends_on = [oci_core_instance.webserver,
    ...
    oci_core_route_table.vcn01_nat_lpg1_route_table,
  ...
  target_resource_details {
    ...
    target_resource_id = oci_core_instance.webserver[count.index].id

    #Optional
    target_resource_operating_system_user_name = "opc"
    target_resource_port                       = 22
    target_resource_private_ip_address         = oci_core_instance.webserver[count.index].private_ip
  }

  display_name           = "ssh_via_bastion_service_to_webserver${count.index}"
  key_type               = "PUB"
